...
public class ChessMatch {
	...
	//SERVIRÁ PARA IMPRIMIR OS MOVIMENTOS POSSIVEIS APARTIR DA POSIÇÃO DE ORIGEM
	public boolean[][] possibleMoves(ChessPosition sourcePosition){
		Position position = sourcePosition.toPosition();
		validateSourcePosition(position);
		return board.piece(position).possibleMoves();
	}
	//METODO PARA MOVER UMA PEÇA
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		//TESTE SE O JOGADOR SE COLOU EM POSIÇÃO DE "XEQUE"
...
//       #MOVIMENTO ESPECIAL "PROMOÇÃO"
		...
		//EXPRESSÃO CONDICIONAL TERNARIA PARA SABER SE O OPONENTE ESTÁ EM POSIÇÃO DE "XEQUE"
...
//       #SPECIALMOVE CASTLING QUEENSIDE ROOK
	...
	private void validateTaegetPosition(Position source, Position target) {
	...
	//TESTA SE EXISTE UM REI NA POSIÇÃO DE "XEQUE"
	private boolean testCheck(Color color) {
		Position kingPosition = King(color).getChessPosition().toPosition();
		List <Piece> opponentPieces = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == opponent(color)).collect(Collectors.toList());
		//PARA CADA PEÇA "P" NA LISTA "OPPONENTPIECES"
	...
	//REALIZA TEST DO  "XEQUE MATE"
	private boolean testCheckMate(Color color) {
		//TESTA SE A COR DA PEÇA NÃO ESTÁ EM XEQUE
		if (!testCheck(color)) {
			return false;
		}
		List <Piece> list = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
		//PERCORRE OS MOVIMENTOS POSSIVEIS DA PEÇA P
		for (Piece p : list) {
			boolean[][] mat = p.possibleMoves();
			for (int i=0;i < board.getRows(); i++) {
				for (int j=0; j < board.getColumns(); j++) {
					//TESTA SE O MOVIMENTO POSSIVEL TIRA DO "XEQUE"
					if(mat[i][j]) {
